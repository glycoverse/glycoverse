...
glycoverse_attach_message <- function(to_load) {
  if (length(to_load) == 0) {
    return(NULL)
  }
  header <- cli::rule(
    left = cli::style_bold("Attaching core glycoverse packages"),
    right = paste0("glycoverse ", package_version_h("glycoverse"))
  )
